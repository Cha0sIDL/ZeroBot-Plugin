...
func init() { // 插件主体
	...
	checkSession := func(ctx *zero.Ctx) bool {
		...
		case "创建轮盘":
			...
			if ss.IsValid {
				...
				ctx.SendChain(message.Text("轮盘游戏已经开始了"))
				return false
			}
		default:
			...
			if ss.IsValid {
				if ss.isExpire() {
					ctx.SendChain(message.Text("轮盘游戏已过期, 请重新开始"))
					ss.close()
					return false
				}
				ctx.SendChain(message.Text("轮盘游戏已经开始了"))
	...
	// 创建轮盘
	engine.OnFullMatch(`创建轮盘`, zero.OnlyGroup, checkSession).SetBlock(true).
	...
	engine.OnFullMatch("加入轮盘", zero.OnlyGroup, checkSession).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			if ss.countUser() >= int(ss.Max) {
				ctx.SendChain(message.Text("目前已有", ss.countUser(), "位玩家, 已达人数上限, 发送\"开始轮盘\"进行游戏"))
				return
			}
			ss.addUser(uid)
			ctx.SendChain(message.Text("成功加入,目前已有", ss.countUser()+1, "位玩家,发送\"开始轮盘\"进行游戏"))
		})

	// 开始轮盘
	engine.OnFullMatch("开始轮盘", zero.OnlyGroup, checkSession).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			uid := ctx.Event.UserID
