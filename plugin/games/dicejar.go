...
func init() {
	...
	engine, gameManager, err := gamesystem.Register("骰子壶", &gamesystem.GameInfo{
		Command: "- 骰子壶@对方QQ",
		Help:    "保证通宝大于20的双方玩家各自投掷1个骰子。\n平局的场合再掷一次直到数目不一样。",
		Rewards: "投掷出来的数目低的玩家将另一方投掷出的数目x2的通宝交给对方。\n" +
			"如果输给投掷出来的数目为6的场合,移交的通宝变成20。",
	})
	if err != nil {
		panic(err)
	}
	engine.OnRegex(`^骰子壶\s*?\[CQ:at,qq=(\d+).*`, zero.OnlyGroup, func(ctx *zero.Ctx) bool {
		...
		if userScore < 20 {
			ctx.SendChain(message.Text("你的通宝不足以满足该游戏"))
			return
		}
		challengScore := wallet.GetWalletOf(duelUser)
		if challengScore < 20 {
			ctx.SendChain(message.Text("他的通宝不足以满足该游戏"))
			return
		}
		// 等待对方响应
		...
		ctx.SendChain(message.At(uid), message.Text("恭喜你赢得了「骰子壶」游戏,获得"), message.At(duelUser), message.Text("的 ", points, " 通宝"))
	})
}
